...
data Obstaculo = Nenhum -- ^ a ausência de obstáculos
...
data Mapa = Mapa Largura [(Terreno, [Obstaculo])] deriving (Show, Read, Eq)

-- | Par de coordenadas de uma posição no 'Mapa'.
type Coordenadas = (Int, Int)

-- | O Jogador define o personagem controlado no 'Jogo'.
newtype Jogador = Jogador Coordenadas deriving (Show, Read, Eq)

-- | Definição base de um jogo.
data Jogo = Jogo Jogador Mapa deriving (Show, Read, Eq)
...
data Jogada = Parado -- ^ tipo que define a ausência de uma acção do 'Jogador'
