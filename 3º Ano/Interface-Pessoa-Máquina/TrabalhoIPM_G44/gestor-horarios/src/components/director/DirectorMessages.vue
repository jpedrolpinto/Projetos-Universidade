...
// Emails (ajustar para usar o email do diretor)
const directorEmail = computed(() => {
  return directorData.value?.email || localStorage.getItem('userEmail') || '';
});
...
// Processar mensagens após carregamento
function processMessages(data, directorData) {
  // Email do diretor
  const directorEmail = directorData.email;
  const directorId = directorData.id;

  return data
    .filter(msg => {
      // Condições para incluir a mensagem
      const isToDirector = msg.to?.toLowerCase() === directorEmail.toLowerCase();
      const isFromDirector = msg.from?.toLowerCase() === directorEmail.toLowerCase();
      const hasDirectorId = msg.directorId === directorId;
      const isDirectorSenderType = msg.senderType === 'director';

      return isToDirector || 
             isFromDirector || 
             hasDirectorId || 
             isDirectorSenderType;
    })
    .map(msg => ({
      ...msg,
      // Considerar como recebida se for para o diretor
      isReceived: msg.to?.toLowerCase() === directorEmail.toLowerCase()
    }))
...
async function loadMessages() {
  try {
    ...
    // Primeiro, garantir que os dados do diretor estão carregados
    if (!directorData.value) {
    ...
    // Processar mensagens utilizando os dados do diretor
    messages.value = processMessages(
...
// Enviar mensagem - versão otimizada
async function sendMessage() {
  ...
  try {
    // Referências para os dados a serem carregados
    const directors = ref([]);
    const teachers = ref([]);
    const students = ref([]);
...
<template>
  <HeaderSidebar 
    ...
    userType="diretor"
    activeMenuItem="mensagensDiretor"
  >
  <div class="messages-container">
    ...
    <div class="tabs-container">
      <div class="tabs">
        <button 
          @click="activeTab = 'received'; loadMessages()" 
        ...
        <tbody>
          ...
          <tr v-else-if="error">
            <td colspan="4" class="error-cell">
              {{ error }}
              <button @click="loadMessages" class="retry-button">Tentar novamente</button>
            </td>
          </tr>
